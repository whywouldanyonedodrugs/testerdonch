...
class Backtester:
    ...
    # ============================
    # META: TRAINING FEATURE STORE (replay exact filled features)
    # ============================

    def _meta_store_path(self) -> Path | None:
        # Prefer explicit config, else default used by 04_models_cv.py
        p = getattr(cfg, "META_TRADE_FEATURES_PATH", None)
        if p:
            pp = Path(p).expanduser().resolve()
            return pp if pp.exists() else None

        # Default training input
        pp = Path("results/trades.clean.csv").resolve()
        if pp.exists():
            return pp
    ...
    def _meta_store_load(self) -> None:
        """
        Loads a keyed store of the exact features used in training (post backfill).
        Index: (symbol_upper, entry_ts_5m)
        If duplicate keys exist, we'll disambiguate by closest entry price at lookup.
        """
        if getattr(self, "_meta_store_df", None) is not None:
            return
        ...
        if path is None:
            self._meta_store_df = None
            self._meta_store_cols = []
            print("[meta_store] no trades.clean.csv found; feature replay disabled", flush=True)
            return

        # Read manifest raw cols so we only keep what the model expects
        model_dir = Path(getattr(cfg, "META_MODEL_DIR", "results/meta_export")).resolve()
        man_path = model_dir / "feature_manifest.json"
        raw_cols = []
        try:
            ...
            print(f"[meta_store] WARNING: could not read manifest {man_path}: {e!r}", flush=True)

        # Determine entry_ts column name
        # (training pipeline expects entry_ts, but be robust)
        if path.suffix.lower() == ".parquet":
        ...
        for c in ("entry_ts", "entry_time", "entry_timestamp", "entry_datetime"):
            if c in cols:
                ...
                break
        if entry_col is None:
            raise RuntimeError(f"[meta_store] {path} missing entry_ts column")

        if "symbol" not in cols:
            raise RuntimeError(f"[meta_store] {path} missing symbol column")

        # Columns we will load
        keep = {"symbol", entry_col}
        if "entry" in cols:
            keep.add("entry")
        # Keep only manifest features that actually exist in the file
        for c in raw_cols:
            if c in cols:
                keep.add(c)
        ...
        df[entry_col] = pd.to_datetime(df[entry_col], utc=True, errors="coerce")
        df = df.dropna(subset=[entry_col, "symbol"])
        ...
        self._meta_store_df = df
        self._meta_store_cols = [c for c in df.columns if c not in ("entry",)]
        print(f"[meta_store] loaded {len(df):,} rows from {path} cols={len(df.columns)}", flush=True)

    def _meta_store_lookup(self, sym: str, ts: pd.Timestamp, entry_price: float | None = None) -> Dict[str, float]:
        """
        Returns a dict of training feature values (exact replay) for this (symbol, entry_ts).
        If multiple rows match the same key, pick the one with closest entry price.
        """
        self._meta_store_load()
        df = getattr(self, "_meta_store_df", None)
        ...
        if isinstance(rows, pd.Series):
            ...
            # DataFrame (duplicate keys)
            if (entry_price is not None) and ("entry" in rows.columns):
                ep = pd.to_numeric(rows["entry"], errors="coerce")
                tgt = float(entry_price)
                j = int((ep - tgt).abs().idxmin()) if hasattr(ep, "idxmin") else 0
                row = rows.loc[j]
            else:
                row = rows.iloc[0]
        ...
        # Also pass entry if available (helps if manifest includes entry-derived features)
        if "entry" in row.index:
            out["entry"] = row.get("entry", np.nan)
    ...
    def _meta_extra_features(
        ...
        *,
        ...
        sets = self._meta_build_regime_sets(
            ts=ts,
            s={**dict(s), **extra} if not isinstance(s, dict) else {**s, **extra},
            regime_up=regime_up,
    ...
    def run(self, signals):
        ...
        for s in sig_iter:
            ...
            if thr is not None:
                ...
                if valid and (prob_val < thr_f):
                    self._log_decision(
                        ...
                        decision="skipped",
                        reason="meta_prob",
                        extra={"prob_val": prob_val, "thr": thr_f},
                    )
                    continue
            ...
            # Probe as a CAP (prevents 0.01 * 0.01 collapse)
            if risk_on != 1:
