...
BASE_RUN_ROOT="$ROOT_DIR/results/rebaseline/phase_qlmg_bybit_historical_execution_data_route_20260628_v1"
if [[ -n "${RUN_ROOT:-}" ]]; then
  RUN_ROOT="$RUN_ROOT"
...
EOF
SMOKE_CMD="cd '$ROOT_DIR' && ./.venv/bin/python tools/run_qlmg_bybit_historical_execution_data_route.py --stage all --smoke --max-symbols 5 --start 2025-01-01 --end 2025-02-15 --disable-telegram --run-root '${RUN_ROOT}_smoke'"
FULL_CMD="cd '$ROOT_DIR' && ./.venv/bin/python tools/run_qlmg_bybit_historical_execution_data_route.py --run-root '$RUN_ROOT' ${ARGS[*]}"
...
if [[ "$LAUNCH_TMUX" -ne 1 ]]; then
  ...
  echo "bash tools/run_qlmg_bybit_historical_execution_data_route_tmux.sh --tmux-session-name $SESSION --stage all --resume --include-d4 --include-listing --include-controls --include-public-trades --include-orderbook --include-liquidations --include-top-of-book --include-shallow-depth --require-telegram --seed 20260628 --launch-tmux" | tee -a "$LOG"
  cat > "$RUN_ROOT/watch_status.json" <<EOF
