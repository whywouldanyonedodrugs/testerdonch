...
def simulate_leg(
    ...
    *,
    ...
    if fill_bar is None or require_utc(fill_bar.open_ts) >= boundary:
        return LegResult("unavailable_boundary_crossing", entry_ts, None, entry_price, None, side, None, None, None, None, None)
    exit_ts = require_utc(fill_bar.open_ts)
    gross = side * (fill_bar.open / entry_price - 1.0) * 10000.0
    funding_bps = _funding_cashflow(funding, entry_ts, exit_ts, side, funding_alignment)
    net = gross - cost_bps + funding_bps
    return LegResult("complete", entry_ts, exit_ts, entry_price, fill_bar.open, side, trigger_reason, gross, cost_bps, funding_bps, net)
...
def contract() -> dict[str, Any]:
    return {
        ...
        "funding": "exact signed cashflow at registered boundary alignment; imputed funding cannot activate signals",
        "trigger": "completed close only",
        "fill": "next authorized trade open with maximum lookup delay enforced by the future cache adapter",
