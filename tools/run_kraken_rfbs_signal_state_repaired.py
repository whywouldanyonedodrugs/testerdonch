#!/usr/bin/env python3
"""Frozen RFBS replay under signal_state_contract_v1_20260715."""
...
RUN_ROOT = Path("results/rebaseline/phase_kraken_rfbs_signal_state_repaired_screen_20260715_v1")
...
def raw_specs(definitions: pd.DataFrame) -> list[dict[str, Any]]:
    rows = definitions[["rally_profile", "confirmation_bars"]].drop_duplicates().sort_values(["rally_profile", "confirmation_bars"])
...
def compact_bundle(root: Path) -> Path:
    files = (
        "RFBS_REPAIRED_RECONCILIATION_REPORT.md", "decision_summary.json", "contract/rfbs_signal_state_contract.md",
        "manifest/riskoff_failed_bounce_definitions.csv", "reproducibility/run_manifest.json",
        ...
        "controls/control_summary.csv", "controls/matched_unmatched_complements.csv", "controls/rally_without_failure_control_comparison.csv",
        "decision/candidate_decisions.csv", "decision/preserved_definition_replay.csv", "candidate_library/rfbs_candidate_library_update.csv",
    )
...
def update_campaign(root: Path, classification: str) -> None:
    ...
    row = preservation.family.eq("RFBS")
    ...
    gate.update({"new_family_launch_allowed": False, "unresolved_registry_count": 2, "directly_affected_roots": 1, "downstream_affected_roots": 1, "next_prompt_target": "RFBS screen and downstream materialization only", "RFBS_lineage_closed": True})
    write_json(gate_path, gate)
    decision_path = CAMPAIGN_ROOT / "decision_summary.json"
    decision = json.loads(decision_path.read_text())
    decision.update({"directly_affected_roots": 1, "downstream_affected_roots": 1, "unresolved_registry_count": 2, "next_recommended_prompt_target": "RFBS screen and downstream materialization only", "RFBS_lineage_closed": True})
...
def run(root: Path, *, resume: bool = False) -> dict[str, Any]:
    ...
    source_definitions = pd.read_csv(SOURCE_ROOT / "manifest/riskoff_failed_bounce_definitions.csv")
        ...
        if resume and shard.exists():
            ...
            if not bars.empty:
                ...
                for spec in raw_specs(definitions):
                    symbol_rows.extend(base.enumerate_raw_signals(ctx, panel, symbol, bars, spec, prepared=prepared))
    ...
    if raw.empty: raise RuntimeError("no parent-neutral RFBS signals")
    ...
    projected, projection_hash = state.project_parent_policies(raw, policies, feature_ts_col="parent_feature_ts", is_allowed=lambda source, policy: (
        str(source["rally_profile"]) == str(policy["rally_profile"])
        and int(source["confirmation_bars"]) == int(policy["confirmation_bars"])
        and base.parent_allowed(str(policy["parent_policy"]), str(source["parent_state"]))
    ))
    ...
    for (extension, confirmation), group in unique_policies.groupby(["rally_profile", "confirmation_bars"]):
        strict_hash = group[group.parent_policy.eq("strict_both_down_stress")].selected_key_policy_hash.iloc[0]
        broad_hash = group[group.parent_policy.eq("broader_fragile_countertrend_stress")].selected_key_policy_hash.iloc[0]
        ...
        nesting.append({"rally_profile": extension, "confirmation_bars": confirmation, "strict_rows": len(strict), "broad_rows": len(broad), "strict_not_in_broad": len(strict-broad), "pass": not bool(strict-broad)})
    nesting = pd.DataFrame(nesting)
    write_csv(root / "audit/raw_parent_policy_nesting.csv", nesting)

    bars_cache, feature_cache, sequence_cache = {}, {}, {}
    for symbol in sorted(projected.symbol.unique()):
        bars = base.runner.load_symbol_bars(paths, symbol, base.START-pd.Timedelta(days=100), base.END)
        bars_cache[symbol] = bars
        prepared = base.prepare_symbol(bars)
        feature_cache[symbol] = prepared[0]
        sequence_cache[symbol] = prepared[3]
    ...
    if accepted.empty: raise RuntimeError("no accepted RFBS trades")
    ...
    outcomes, boundaries = base.common.attach_costs(accepted, funding, "event_id")
    ...
    if len(controls): controls, control_boundaries = base.common.attach_costs(controls, funding, "control_event_id")
    ...
    structural = control_summary[control_summary.control_class.eq("countertrend_rally_without_completed_failure")]
    write_csv(root / "controls/rally_without_failure_control_comparison.csv", structural)

    summary, attribution, period = base.common.summarize_economics(outcomes, definitions)
    concentration = base.lfbs.concentration_forensics(outcomes)
    neighborhood = outcomes.groupby(
        ["rally_profile", "confirmation_bars", "parent_policy", "exit_policy"]
    ).agg(
        events=("event_id", "size"), symbols=("symbol", "nunique"),
        base_mean_R=("net_base_R", "mean"), conservative_mean_R=("net_conservative_R", "mean"),
        severe_mean_R=("net_severe_R", "mean"),
    ).reset_index()
    exits = outcomes.groupby(
        ["rally_profile", "confirmation_bars", "parent_policy", "exit_policy", "exit_reason"]
    ).agg(events=("event_id", "size"), conservative_mean_R=("net_conservative_R", "mean")).reset_index()
    funding_report = base.common.funding_partition_report(outcomes)
    ...
    manifest_mismatch = int(file_hash(root / "manifest/riskoff_failed_bounce_definitions.csv") != file_hash(SOURCE_ROOT / "manifest/riskoff_failed_bounce_definitions.csv"))
    ...
    for definition in definitions.itertuples(index=False):
        ...
        library.append({
            ...
            "hypothesis_id": "riskoff_failed_bounce_short", "family_engine_id": "kraken_rfbs_v1_signal_state_repaired",
    ...
    write_csv(root / "candidate_library/rfbs_candidate_library_update.csv", library)
    ...
    repro = {
        ...
        "source_runner_hash": file_hash(Path(base.__file__)), "config_hash": file_hash(root / "manifest/riskoff_failed_bounce_definitions.csv"), "contract_hash": file_hash(root / "contract/rfbs_signal_state_contract.md"),
    ...
    report = f"""# RFBS Repaired Reconciliation\n\nThe frozen 24-definition screen was replayed under `{CONTRACT_VERSION}` without seven-day selected-key preblocking. Old root: `{SOURCE_ROOT}`. Repaired root: `{root}`. Screen classification: `{final}`. Old addresses unchanged/restored/removed: {int(mapping.address_status.eq('unchanged_address').sum())}/{int(mapping.address_status.eq('restored_or_new_address').sum())}/{int(mapping.address_status.eq('removed_address').sum())}. Preserved variants `rfbs_v1_001`, `rfbs_v1_004`, `rfbs_v1_007`, and `rfbs_v1_010` were replayed explicitly. Prior decisions remain quarantined until the repaired downstream materialization closes.\n"""
    (root / "RFBS_REPAIRED_RECONCILIATION_REPORT.md").write_text(report)
    result = {
    ...
    compact_bundle(root)
    # Campaign closure is owned by the mandatory downstream replay.
