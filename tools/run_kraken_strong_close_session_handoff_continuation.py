#!/usr/bin/env python3
"""Train-only strong-close session-handoff continuation screen v1."""
...
RUN_ROOT = Path("results/rebaseline/phase_kraken_strong_close_session_handoff_continuation_20260716_v1")
REFERENCE_ROOT = Path("results/rebaseline/phase_kraken_failed_breakdown_squeeze_reclaim_long_screen_20260716_v1")
...
CONTRACT_VERSION = "kraken_strong_close_session_handoff_continuation_v1_20260716"
SIGNAL_STATE_CONTRACT_VERSION = signal_state.SIGNAL_STATE_CONTRACT_VERSION
RISK_MATCH_TOLERANCE_ATR = 0.25
CONTROL_CLASSES = (
    "middle_close_same_handoff_range_volume",
    "strong_close_shifted_four_hours",
    "same_handoff_close_location_without_volume_expansion",
    "generic_same_direction_8h_return_continuation",
    "same_symbol_same_parent_random_session_entry",
)
CONTEXTUAL_CONTROLS = {"same_symbol_same_parent_random_session_entry"}
...
def policy_hash(row: Mapping[str, Any], *, include_exit: bool) -> str:
    fields = ["handoff_hour_utc", "direction", "parent_policy"]
    if include_exit:
        fields.append("exit_policy")
    vector = {field: row[field] for field in fields}
    vector.update({
        "session": "completed_8h_utc_session_96x5m_bars",
        "rolling_reference": "prior_20_completed_same_handoff_sessions_shift_1",
        "range_multiple": 1.5, "quote_volume_proxy_multiple": 1.5,
        "body_fraction_minimum": 0.5, "long_clv_minimum": 0.8, "short_clv_maximum": 0.2,
        "execution": "next_executable_5m_open", "stop": "completed_session_midpoint",
...
def frozen_manifest() -> pd.DataFrame:
    rows = []
    for handoff_hour in (8, 16):
        for direction in ("long", "short"):
            for parent_policy in ("directionally_aligned", "all_regime_comparator"):
                for exit_policy in ("fixed_8h", "fixed_24h", "swing_trail_48h"):
                    row = {
                        "definition_id": f"schc_v1_{len(rows)+1:03d}", "handoff_hour_utc": handoff_hour,
                        "direction": direction, "parent_policy": parent_policy,
...
def raw_policy_hash(handoff_hour: int, direction: str) -> str:
    return stable_hash({
        "handoff_hour_utc": int(handoff_hour), "direction": direction,
        "session_semantics": "completed_8h_utc_96x5m",
        "rolling_reference": "prior_20_same_handoff_sessions_shift_1",
...
def _session_grid(work: pd.DataFrame, *, offset_hours: int, grid_name: str) -> pd.DataFrame:
    source = work.copy()
    source["quote_notional_proxy"] = source["typical"] * source["volume"]
    sessions = source.set_index("known_ts").resample(
        "8h", origin="epoch", offset=pd.Timedelta(hours=offset_hours), label="right", closed="right"
    ).agg(
        open=("open", "first"), high=("high", "max"), low=("low", "min"), close=("close", "last"),
        reported_volume=("volume", "sum"), quote_volume_proxy=("quote_notional_proxy", "sum"),
        execution_bar_count=("close", "count"),
    ).dropna(subset=["open", "high", "low", "close"]).reset_index().rename(columns={"known_ts": "decision_ts"})
    sessions = sessions[sessions.execution_bar_count.eq(96)].copy()
    previous_close = sessions.close.shift(1)
    sessions["true_range"] = pd.concat([
        sessions.high - sessions.low,
        (sessions.high - previous_close).abs(),
        (sessions.low - previous_close).abs(),
    ], axis=1).max(axis=1)
    sessions["session_range"] = sessions.high - sessions.low
    sessions["body"] = sessions.close - sessions.open
    sessions["body_fraction"] = sessions.body.abs() / sessions.session_range.replace(0, np.nan)
    sessions["close_location_value"] = (sessions.close - sessions.low) / sessions.session_range.replace(0, np.nan)
    sessions["session_grid"] = grid_name
    sessions["session_end_hour_utc"] = sessions.decision_ts.dt.hour
    for hour in sorted(sessions.session_end_hour_utc.unique()):
        mask = sessions.session_end_hour_utc.eq(hour)
        sessions.loc[mask, "rolling_median_true_range_20"] = sessions.loc[mask, "true_range"].rolling(20, min_periods=20).median().shift(1)
        sessions.loc[mask, "rolling_median_quote_volume_proxy_20"] = sessions.loc[mask, "quote_volume_proxy"].rolling(20, min_periods=20).median().shift(1)
    sessions["range_expansion"] = sessions.true_range.ge(1.5 * sessions.rolling_median_true_range_20)
    sessions["volume_expansion"] = sessions.quote_volume_proxy.ge(1.5 * sessions.rolling_median_quote_volume_proxy_20)
    sessions["long_strong_close"] = sessions.body.gt(0) & sessions.body_fraction.ge(0.5) & sessions.close_location_value.ge(0.8)
    sessions["short_strong_close"] = sessions.body.lt(0) & sessions.body_fraction.ge(0.5) & sessions.close_location_value.le(0.2)
    sessions["middle_close"] = sessions.close_location_value.between(0.3, 0.7)
    return sessions
...
def feature_frames(bars: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    main = _session_grid(work, offset_hours=0, grid_name="handoff")
    shifted = _session_grid(work, offset_hours=4, grid_name="shifted_plus_4h")
    sessions = pd.concat([main, shifted], ignore_index=True).sort_values("decision_ts")
    daily_fields = daily[["daily_source_ts", "atr_14d", "ema_10", "close"]].rename(columns={"close": "daily_close"})
    sessions = pd.merge_asof(
        sessions, daily_fields.sort_values("daily_source_ts"),
        left_on="decision_ts", right_on="daily_source_ts", direction="backward", allow_exact_matches=True,
    )
    sessions = parent.attach_parent_state(sessions)
    sessions["feature_available_ts"] = sessions[["decision_ts", "daily_source_ts", "parent_source_ts"]].max(axis=1)
    return sessions, work, daily, four
...
def enumerate_raw_signals(ctx: Any, panel: pd.DataFrame, symbol: str, bars: pd.DataFrame, prepared: tuple | None = None) -> tuple[list[dict[str, Any]], list[dict[str, Any]], pd.DataFrame, pd.DataFrame]:
    sessions, _, _, four = prepared or prepare_symbol(bars)
    panel_row = panel[panel.symbol.eq(symbol)]
    if panel_row.empty or str(panel_row.iloc[0].status) != "available":
        return [], [], sessions, four
    listed = pd.Timestamp(panel_row.iloc[0].start_ts)
    rows: list[dict[str, Any]] = []
    drops: list[dict[str, Any]] = []
    main = sessions[sessions.session_grid.eq("handoff") & sessions.session_end_hour_utc.isin([8, 16])]
    for handoff_hour in (8, 16):
        for direction in ("long", "short"):
            raw_hash = raw_policy_hash(handoff_hour, direction)
            mask = main.session_end_hour_utc.eq(handoff_hour) & main.range_expansion & main.volume_expansion
            mask &= main.long_strong_close if direction == "long" else main.short_strong_close
            for decision in main[mask].itertuples(index=False):
                ...
                if entry_ts >= PROTECTED:
                    ...
                    continue
                midpoint = (float(decision.high) + float(decision.low)) / 2.0
                risk = float(entry.open) - midpoint if direction == "long" else midpoint - float(entry.open)
                ...
                row = {
                    "raw_policy_hash": raw_hash, "symbol": symbol, "handoff_hour_utc": handoff_hour,
                    ...
                    "entry_ts": entry_ts, "entry_price": float(entry.open), "initial_stop": midpoint,
                    "risk_denominator": risk, "risk_to_daily_atr": risk_atr, "daily_atr": float(decision.atr_14d),
                    "session_open": decision.open, "session_high": decision.high, "session_low": decision.low,
                    "session_close": decision.close, "session_true_range": decision.true_range,
                    "session_quote_volume_proxy": decision.quote_volume_proxy,
                    "rolling_median_true_range_20": decision.rolling_median_true_range_20,
                    "rolling_median_quote_volume_proxy_20": decision.rolling_median_quote_volume_proxy_20,
                    "body_fraction": decision.body_fraction, "close_location_value": decision.close_location_value,
                    "evaluation_period": period, "evaluation_window_start": window_start, "evaluation_window_end": window_end,
                    "imputed_funding_gate_activated": False, "ohlcv_quote_volume_proxy_cap": True,
                }
                row["raw_signal_address_hash"] = raw_signal_address(row)
                row["raw_signal_id"] = "SCHCRAW_" + row["raw_signal_address_hash"][:24]
                rows.append(row)
    return rows, drops, sessions, four
...
def project_parent_policies(raw: pd.DataFrame, definitions: pd.DataFrame) -> pd.DataFrame:
    ...
    for policy in definitions.drop_duplicates("selected_key_policy_hash").to_dict("records"):
        selected = raw[(raw.handoff_hour_utc == policy["handoff_hour_utc"]) & (raw.direction == policy["direction"])]
        selected = selected[selected.parent_state.map(lambda value: directional_parent_allowed(policy["parent_policy"], policy["direction"], str(value)))]
        for source in selected.to_dict("records"):
            ...
            row["candidate_key"] = "SCHCK_" + stable_hash({"policy": policy["selected_key_policy_hash"], "raw": source["raw_signal_address_hash"]})[:24]
            rows.append(row)
...
def simulate_definition(candidates: pd.DataFrame, definition: Mapping[str, Any], execute_fn: Callable) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for key in candidates.sort_values(["entry_ts", "symbol", "candidate_key"]).to_dict("records"):
        ...
        event["event_id"] = "SCHCE_" + stable_hash({"candidate": key["candidate_key"], "definition": definition["definition_id"]})[:24]
...
def execute_event_indexed(key: Mapping[str, Any], exit_policy: str, data: dict[str, Any]) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    ...
    hours = 8 if exit_policy == "fixed_8h" else 24 if exit_policy == "fixed_24h" else 48
    ...
    frame_end = int(np.searchsorted(data["frame_ts"], limit64, side="left"))
    relevant = data["frame"].iloc[frame_start:frame_end].reset_index(drop=True)
    if exit_policy == "swing_trail_48h":
        segment_start = start
        for position in range(2, len(relevant)):
            ...
            prior_fill = _first_stop_fill(data, segment_start, min(update_index, end), stop, side)
            if prior_fill is not None:
                fill = prior_fill; reason = "session_midpoint_or_swing_stop"; break
            middle = relevant.iloc[position-1]
            if side == "long" and float(middle.low) < float(relevant.iloc[position-2].low) and float(middle.low) <= float(relevant.iloc[position].low):
                stop = max(stop, float(middle.low))
            elif side == "short" and float(middle.high) > float(relevant.iloc[position-2].high) and float(middle.high) >= float(relevant.iloc[position].high):
                stop = min(stop, float(middle.high))
            segment_start = max(segment_start, update_index)
        if fill is None:
            fill = _first_stop_fill(data, segment_start, end, stop, side)
            if fill is not None: reason = "session_midpoint_or_swing_stop"
    else:
        fill = _first_stop_fill(data, start, end, stop, side)
        if fill is not None: reason = "completed_session_midpoint_stop"
    if fill is None:
...
def execute_event_scalar(key: Mapping[str, Any], exit_policy: str, bars: pd.DataFrame, four: pd.DataFrame) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    ...
    relevant = four[(four.decision_ts > entry_ts) & (four.decision_ts < limit)].sort_values("decision_ts").reset_index(drop=True)
    stop = float(key["initial_stop"]); side = str(key["side"]); update_position = 2
    exit_ts = pd.NaT; exit_price = np.nan; reason = ""
    for bar in path.itertuples(index=False):
        if exit_policy == "swing_trail_48h":
            while update_position < len(relevant) and pd.Timestamp(relevant.iloc[update_position].decision_ts) <= pd.Timestamp(bar.ts):
                before, middle, after = relevant.iloc[update_position-2], relevant.iloc[update_position-1], relevant.iloc[update_position]
                if side == "long" and float(middle.low) < float(before.low) and float(middle.low) <= float(after.low):
                    stop = max(stop, float(middle.low))
                elif side == "short" and float(middle.high) > float(before.high) and float(middle.high) >= float(after.high):
                    stop = min(stop, float(middle.high))
                update_position += 1
        ...
        if hit:
            ...
            reason = "session_midpoint_or_swing_stop" if exit_policy == "swing_trail_48h" else "completed_session_midpoint_stop"
            break
    if pd.isna(exit_ts):
...
def raw_nesting_audit(raw: pd.DataFrame, projected: pd.DataFrame, definitions: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for (handoff_hour, direction), group in definitions.drop_duplicates("selected_key_policy_hash").groupby(["handoff_hour_utc", "direction"]):
        ...
        raw_set = set(raw[(raw.handoff_hour_utc == handoff_hour) & (raw.direction == direction)].raw_signal_address_hash)
        rows.append({"handoff_hour_utc": handoff_hour, "direction": direction, "raw_rows": len(raw_set), "aligned_rows": len(strict), "all_regime_rows": len(broad), "aligned_not_in_all": len(strict-broad), "all_missing_known_raw": len(raw_set-broad), "pass": strict <= broad and broad == raw_set})
    return pd.DataFrame(rows)
...
def build_control_pool(frame: pd.DataFrame, data: dict[str, Any], side: str) -> pd.DataFrame:
    """Construct outcome-blind session controls using the candidate midpoint risk rule."""
    pool = frame.copy().sort_values(["decision_ts", "session_grid"]).reset_index(drop=True)
    pool["initial_stop"] = (pool.high + pool.low) / 2.0
    ...
    pool["risk_denominator"] = (
        pool.entry_price - pool.initial_stop if side == "long" else pool.initial_stop - pool.entry_price
    )
...
def _pool_choice(
    ...
    fields = (
        ...
        "session_grid", "session_end_hour_utc", "close_location_value", "range_expansion",
        "volume_expansion", "long_strong_close", "short_strong_close",
    )
...
def build_control_keys(
    ...
    for symbol in feature_cache:
        for side in ("long", "short"):
            pools[(symbol, side)] = build_control_pool(feature_cache[symbol], indexed_cache[symbol], side)
    ...
    for number,key in enumerate(candidates.itertuples(index=False),1):
        if key.candidate_key not in policies:
            continue
        pool = pools[(key.symbol, key.side)]
        candidate_risk = float(key.risk_to_daily_atr)
        proposals: dict[str, list[dict[str, Any]]] = {name: [] for name in CONTROL_CLASSES}
        main = pool.session_grid.eq("handoff") & pool.session_end_hour_utc.eq(int(key.handoff_hour_utc))
        shifted = pool.session_grid.eq("shifted_plus_4h") & pool.session_end_hour_utc.eq((int(key.handoff_hour_utc) + 4) % 24)
        directional = pool.long_strong_close if key.side == "long" else pool.short_strong_close
        masks = {
            "middle_close_same_handoff_range_volume": main & pool.range_expansion & pool.volume_expansion & pool.middle_close,
            "strong_close_shifted_four_hours": shifted & pool.range_expansion & pool.volume_expansion & directional,
            "same_handoff_close_location_without_volume_expansion": main & pool.range_expansion & ~pool.volume_expansion & directional,
            "generic_same_direction_8h_return_continuation": main & ((pool.close > pool.open) if key.side == "long" else (pool.close < pool.open)),
            "same_symbol_same_parent_random_session_entry": main & pool.parent_state.eq(key.parent_state),
        }
        ...
        for control_class, options in proposals.items():
            ...
            for definition in policies[key.candidate_key].itertuples(index=False):
                ...
                hours = 8 if definition.exit_policy == "fixed_8h" else 24 if definition.exit_policy == "fixed_24h" else 48
                ...
                control_key = "SCHCC_" + stable_hash(vector)[:24]
    ...
    for definition_id in sorted(events.definition_id.unique()):
        ...
        for control_class in CONTROL_CLASSES:
            ...
            for mode in ("base", "conservative", "severe"):
                ...
                if control_class in {
                    "middle_close_same_handoff_range_volume", "strong_close_shifted_four_hours",
                    "same_handoff_close_location_without_volume_expansion", "generic_same_direction_8h_return_continuation",
                }:
                    ...
                    component = {
                        "middle_close_same_handoff_range_volume": "strong_close_location",
                        "strong_close_shifted_four_hours": "handoff_timing",
                        "same_handoff_close_location_without_volume_expansion": "volume_expansion",
                        "generic_same_direction_8h_return_continuation": "full_strong_close_contract",
                    }[control_class]
...
def update_central_artifacts(root: Path, library: pd.DataFrame, final_decision: str) -> None:
    ...
    write_json(root / "continuity/continuity_state_snapshot.json", {
        ...
        "family": "strong_close_session_handoff_continuation",
...
def compact_bundle(root: Path) -> Path:
    files = (
        "SCREEN_REPORT.md", "decision_summary.json", "contract/strong_close_session_handoff_contract.md",
...
def run(root: Path) -> dict[str, Any]:
    ...
    contract_text = """# Strong-Close Session-Handoff Continuation v1 Contract

Train-only 2023-2025. At 08:00 and 16:00 UTC, aggregate the just-completed eight-hour session from exactly 96 five-minute bars. Rolling range and volume references use only the prior 20 sessions ending at the corresponding UTC handoff (`rolling.median().shift(1)`). True range and an explicitly capped OHLCV quote-notional proxy must each be at least 1.5 times their reference median; the directional body must be at least half the session range and close-location value must be >=0.80 long or <=0.20 short. Because local historical candles do not identify exact quote-volume units, `sum(typical_price * reported_volume)` is used only as a relative volume-expansion proxy and every result carries `ohlcv_quote_volume_proxy_cap`.

Entry is the next executable five-minute open after handoff; stop is the completed session midpoint and initial risk must be 0.25-1.5 completed-daily ATR. Parent-neutral raw tapes are frozen before PIT directionally-aligned/all-regime projections. Non-overlap is definition-local and uses actual executable exits. Funding and OI never activate a signal. Evaluation-boundary crossings are excluded, never artificially exited. Controls are selected before outcomes with the same risk band and a frozen +/-0.25 daily-ATR distance rule.
"""
    contract_path = root / "contract/strong_close_session_handoff_contract.md"; contract_path.parent.mkdir(); contract_path.write_text(contract_text, encoding="utf-8")
    ...
    for number, symbol in enumerate(panel.symbol.astype(str), 1):
        ...
        rows, drops, sessions, four = enumerate_raw_signals(ctx, panel, symbol, bars, prepared)
        raw_rows.extend(rows); raw_drops.extend(drops)
        if rows:
            feature_cache[symbol] = sessions; four_cache[symbol] = four; bars_cache[symbol] = bars
    ...
    dimension_rows = []
    for fields in (("direction",), ("handoff_hour_utc",), ("parent_policy",), ("exit_policy",)):
    ...
    neighborhood = outcomes.groupby(["handoff_hour_utc","direction","parent_policy","exit_policy"]).agg(events=("event_id","size"),symbols=("symbol","nunique"),months=("entry_ts",lambda x:pd.to_datetime(x,utc=True).dt.strftime("%Y-%m").nunique()),base_mean_R=("net_base_R","mean"),conservative_mean_R=("net_conservative_R","mean"),severe_mean_R=("net_severe_R","mean")).reset_index(); write_csv(root / "forensics/parameter_neighborhood.csv", neighborhood)
    ...
    hard = {
        ...
        "quote_volume_proxy_cap_propagation_failures": int((~raw.ohlcv_quote_volume_proxy_cap.astype(bool)).sum()) + int((~outcomes.ohlcv_quote_volume_proxy_cap.astype(bool)).sum()),
