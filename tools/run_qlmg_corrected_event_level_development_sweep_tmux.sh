...
cd "$ROOT"
SESSION="qlmg_corrected_dev_sweep"
LAUNCH=0
SKIP_SMOKE=0
EXPLICIT_RUN_ROOT=""
REQUIRE_TELEGRAM=0
...
if [[ "$LAUNCH" != "1" ]]; then
  ...
  echo "Suggested command: bash tools/run_qlmg_corrected_event_level_development_sweep_tmux.sh --tmux-session-name ${SESSION} --stage all --resume --include-a3 --include-a2-redesign --include-b1-ledger-sidecar --include-c2-ledger-sidecar --include-branch-x-status --corrected-sweep-budget 6000 --a3-budget 2500 --a2-budget 2000 --b1-sidecar-budget 900 --c2-sidecar-budget 600 --nulls-per-event 3 --top-per-family 60 --aggressive-overlay --require-telegram --seed 20260629 --launch-tmux"
  exit 0
fi
...
FULL_CMD=("$ROOT/.venv/bin/python" "tools/run_qlmg_corrected_event_level_development_sweep.py" "--run-root" "$RUN_ROOT" "${ARGS[@]}")
SMOKE_CMD=("$ROOT/.venv/bin/python" "tools/run_qlmg_corrected_event_level_development_sweep.py" "--stage" "all" "--smoke" "--max-symbols" "5" "--start" "2025-01-01" "--end" "2025-02-15" "--disable-telegram" "--corrected-sweep-budget" "80" "--a3-budget" "30" "--a2-budget" "30" "--b1-sidecar-budget" "10" "--c2-sidecar-budget" "10" "--nulls-per-event" "1" "--top-per-family" "5")
