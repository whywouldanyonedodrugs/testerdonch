...
if [[ "$LAUNCH" != "1" ]]; then
  echo "Full tmux launch not started. Re-run with --launch-tmux."
  echo "Default full command includes --download-official-data --download-cap-gb 25."
  exit 0
fi
...
{"status":"launching","session":"$SESSION","run_root":"$RUN_ROOT","resource_mode":"nice_ionice"}
JSON

FULL_CMD=("$ROOT/.venv/bin/python" "tools/run_kraken_k0_data_foundation.py" "--run-root" "$RUN_ROOT" "${ARGS[@]}")
SMOKE_CMD=("$ROOT/.venv/bin/python" "tools/run_kraken_k0_data_foundation.py" "--stage" "all" "--smoke" "--max-symbols" "5" "--start" "2025-01-01" "--end" "2025-02-15" "--disable-telegram" "--download-cap-gb" "1")

if command -v ionice >/dev/null 2>&1; then
  PREFIX="nice -n 10 ionice -c2 -n7"
else
  PREFIX="nice -n 10"
fi

if [[ "$SKIP_SMOKE" == "1" ]]; then
  INNER="set -euo pipefail; echo '[tmux] run root: $RUN_ROOT'; echo '[tmux] resource mode: $PREFIX'; mkdir -p '$RUN_ROOT/logs'; $PREFIX ${FULL_CMD[*]} 2>&1 | tee '$RUN_ROOT/logs/full_run.log'"
else
  INNER="set -euo pipefail; echo '[tmux] run root: $RUN_ROOT'; echo '[tmux] smoke first'; mkdir -p '$RUN_ROOT/logs'; ${SMOKE_CMD[*]} 2>&1 | tee '$RUN_ROOT/logs/smoke_run.log'; echo '[tmux] full run resource mode: $PREFIX'; $PREFIX ${FULL_CMD[*]} 2>&1 | tee '$RUN_ROOT/logs/full_run.log'"
fi
