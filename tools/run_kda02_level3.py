#!/usr/bin/env python3
"""Execute the one conditionally authorized frozen KDA02 v2 Level-3 run."""
...
def preflight(archive: Path, review_path: Path) -> tuple[dict[str, Any], pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    contract_path = archive / "KDA02_FINAL_LEVEL3_CONTRACT.json"
    definitions_path = archive / "KDA02_LEVEL3_DEFINITION_REGISTER.csv"
    event_path = archive / "KDA02_V2_EVENT_TAPE.parquet"
    parent_path = archive / "KDA02_V2_PARENT_EPISODE_TAPE.parquet"
    gate_path = archive / "KDA02_V2_FEASIBILITY_GATES.csv"
    contract = json.loads(contract_path.read_text())
    if not review.get("approved"):
        raise ValueError("independent KDA02 pre-outcome approval missing")
    ...
    paths = {
        ...
        "feature_module": ROOT / "tools/qlmg_kda02_v2.py",
        "builder": ROOT / "tools/build_kda02_v2_prerun_freeze.py",
        "calculation_module": ROOT / "tools/qlmg_kda02_level3.py",
    ...
    if contract.get("contract_version") != "kda02_v2_level3_contract_v1_20260719":
        raise ValueError("unexpected KDA02 frozen contract version")
    if contract.get("controls_executed") is not False or contract.get("kda02b_outcomes_executed") is not False:
        raise ValueError("control/KDA02B authorization mismatch")
    ...
    if int(feasible.sum()) < 2 or definitions.empty:
        raise ValueError("KDA02 conditional economic gate is not open")
    if events.event_id.duplicated().any() or events.economic_address.duplicated().any():
        raise ValueError("duplicate frozen KDA02 event identity")
    ...
    if (events.decision_ts < TRAIN_START).any() or (events.decision_ts >= PROTECTED_START).any():
        raise ValueError("non-rankable frozen KDA02 event")
...
def price_and_score(
    ...
    if len(trades) and (
        ...
        raise ValueError("duplicate or protected KDA02 trade outcome")
...
def attach_funding_diagnostics(
    ...
    if trades.level3_economic_address.duplicated().any():
        raise ValueError("duplicate KDA02 Level-3 address before funding")
    ...
    if result.level3_economic_address.duplicated().any() or "candidate_economic_address" not in result:
        raise ValueError("KDA02 funding identity round-trip failed")
...
def reports(definitions: pd.DataFrame, records: pd.DataFrame, trades: pd.DataFrame):
    ...
    metrics, gates, bootstrap, distributions, concentration, decisions, sensitivities = [], [], [], [], [], [], []
    for definition in definitions.sort_values(["robustness_only", "definition_id"], kind="mergesort").to_dict("records"):
        ...
        row = {
            **definition,
            "candidate_count": int(records.definition_id.eq(did).sum()),
            "accepted_count": len(trade),
            "trades_2023": int(years.get(2023, 0)),
            "trades_2024": int(years.get(2024, 0)),
            "trades_2025": int(years.get(2025, 0)),
            "symbols": int(trade.symbol.nunique()),
            "market_day_clusters": len(daily),
            ...
            "equal_day_base_mean_bps": float(daily.base_net_bps.mean()),
            ...
            "bootstrap_lower_bps": low,
            "bootstrap_upper_bps": high,
        ...
        bootstrap.append({
            "definition_id": did, "market_day_clusters": len(daily),
            "resamples": BOOTSTRAP_RESAMPLES, "seed": BOOTSTRAP_SEED,
            "lower_bps": low, "upper_bps": high,
        })
        concentration.append({
            "definition_id": did,
            "market_day_positive_share": row["market_day_positive_share"],
            "symbol_positive_share": row["symbol_positive_share"],
            "year_positive_share": row["year_positive_share"],
        })
        decisions.append({
            "definition_id": did,
            "attempt": definition["attempt"],
            "all_gates_pass": flags["all_gates_pass"],
            "decision": (
                "primary_pass_controls_required"
                if definition["attempt"] == "primary" and flags["all_gates_pass"]
                else "no_primary_pass"
                if definition["attempt"] == "primary"
                else "robustness_diagnostic_only"
            ),
        })
        for cluster in ("market_6h_cluster_id", "parent_episode_id"):
            ...
            sensitivities.append({
                ...
                "bootstrap_lower_bps": sensitivity_low,
                "bootstrap_upper_bps": sensitivity_high,
            })
    return tuple(pd.DataFrame(value) for value in (metrics, gates, bootstrap, distributions, concentration, decisions, sensitivities)), day, weighted
...
def main() -> int:
    ...
    if not args.execute_frozen_economic_run:
        raise ValueError("explicit frozen KDA02 economic authorization flag required")
    if args.output.exists():
        raise ValueError("fresh KDA02 economic output root required")
    ...
    accepted.to_parquet(args.output / "KDA02_LEVEL3_ACCEPTED_EXECUTIONS.parquet", index=False, compression="zstd")
    records.loc[~records.accepted].to_parquet(args.output / "KDA02_LEVEL3_EXECUTION_REJECTIONS.parquet", index=False, compression="zstd")
    trades, price_rejections = price_and_score(accepted, events, authority)
    if len(price_rejections):
        raise ValueError("KDA02 exact-open price rejection")
    ...
    metrics, gates, bootstrap, distributions, concentration, decisions, sensitivity = outputs
    primary_pass = decisions.loc[decisions.attempt.eq("primary") & decisions.all_gates_pass]
    terminal = "KDA02_level3_primary_pass_controls_required" if len(primary_pass) else "KDA02_level3_no_primary_pass_stop"
    weighted.to_parquet(args.output / "KDA02_LEVEL3_TRADE_TAPE.parquet", index=False, compression="zstd")
    metrics.to_csv(args.output / "KDA02_LEVEL3_DEFINITION_METRICS.csv", index=False)
    gates.to_csv(args.output / "KDA02_LEVEL3_GATE_MATRIX.csv", index=False)
    day.to_parquet(args.output / "KDA02_LEVEL3_MARKET_DAY_RETURNS.parquet", index=False, compression="zstd")
    bootstrap.to_csv(args.output / "KDA02_LEVEL3_BOOTSTRAP_SUMMARY.csv", index=False)
    distributions.to_parquet(args.output / "KDA02_LEVEL3_BOOTSTRAP_DISTRIBUTIONS.parquet", index=False, compression="zstd")
    concentration.to_csv(args.output / "KDA02_LEVEL3_CONCENTRATION.csv", index=False)
    sensitivity.to_csv(args.output / "KDA02_LEVEL3_CLUSTER_SENSITIVITY.csv", index=False)
    decisions.to_csv(args.output / "KDA02_LEVEL3_DEFINITION_DECISIONS.csv", index=False)
    ...
    ).to_csv(args.output / "KDA02_LEVEL3_FUNDING_PARTITIONS.csv", index=False)
    boundaries.to_parquet(args.output / "KDA02_LEVEL3_FUNDING_BOUNDARY_LEDGER.parquet", index=False, compression="zstd")
    (args.output / "KDA02_LEVEL3_DECISION.md").write_text(
        f"# KDA02 Level-3 Decision\n\n`{terminal}`\n\nPassing primary definitions: `{len(primary_pass)}`. "
        "All passing primaries remain frozen; controls and KDA02B outcomes were not executed.\n",
        encoding="utf-8",
    )
    (args.output / "KDA02_LEVEL3_CLAIM_BOUNDARY.md").write_text(
        "# KDA02 Level-3 Claim Boundary\n\nKraken train-only, current-roster/lifecycle-capped evidence under inferred "
        "analytics units. Liquidation direction remains a price proxy. Mark confirms state but is not a fill. "
    ...
    audit = {
        ...
        "contract_file_sha256": sha256_file(args.archive / "KDA02_FINAL_LEVEL3_CONTRACT.json"),
        "definition_register_sha256": sha256_file(args.archive / "KDA02_LEVEL3_DEFINITION_REGISTER.csv"),
        "event_tape_sha256": sha256_file(args.archive / "KDA02_V2_EVENT_TAPE.parquet"),
        "parent_tape_sha256": sha256_file(args.archive / "KDA02_V2_PARENT_EPISODE_TAPE.parquet"),
        "runner_sha256": sha256_file(Path(__file__)),
        "calculation_module_sha256": sha256_file(ROOT / "tools/qlmg_kda02_level3.py"),
        ...
        "passing_primary_definitions": primary_pass.definition_id.tolist(),
