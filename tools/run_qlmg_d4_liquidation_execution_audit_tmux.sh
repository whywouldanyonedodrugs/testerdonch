...
if [[ -z "$RUN_ROOT" ]]; then
  BASE="/opt/testerdonch/results/rebaseline/phase_qlmg_d4_liquidation_execution_audit_20260625_v1"
  if [[ -d "$BASE" ]]; then
...
if [[ "$SKIP_SMOKE" -eq 0 ]]; then
  ./.venv/bin/python tools/run_qlmg_d4_liquidation_execution_audit.py --stage all --smoke --max-symbols 5 --start 2025-01-01 --end 2025-02-15 --disable-telegram --run-root "$RUN_ROOT/smoke" 2>&1 | tee "$RUN_ROOT/logs/smoke.log"
...
./.venv/bin/python tools/run_qlmg_d4_liquidation_execution_audit.py$FULL_ARGS_Q 2>&1 | tee "$RUN_ROOT/logs/full_run.log"
rc=\${PIPESTATUS[0]}
if [[ \$rc -eq 0 ]]; then
