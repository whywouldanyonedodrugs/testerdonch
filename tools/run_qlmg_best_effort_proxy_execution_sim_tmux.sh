...
if [[ "$LAUNCH" != "1" ]]; then
  ...
  echo "Suggested command: bash tools/run_qlmg_best_effort_proxy_execution_sim_tmux.sh --tmux-session-name ${SESSION} --stage all --resume --include-listing --include-controls --include-d4 --require-telegram --seed 20260628 --launch-tmux"
  exit 0
fi
...
FULL_CMD=("$ROOT/.venv/bin/python" "tools/run_qlmg_best_effort_proxy_execution_sim.py" "--run-root" "$RUN_ROOT" "${ARGS[@]}")
SMOKE_CMD=("$ROOT/.venv/bin/python" "tools/run_qlmg_best_effort_proxy_execution_sim.py" "--stage" "all" "--smoke" "--max-symbols" "5" "--start" "2025-01-01" "--end" "2025-02-15" "--disable-telegram")
