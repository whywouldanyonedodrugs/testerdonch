#!/usr/bin/env python3
"""Detached Telegram/status watcher for the DFRL signal-state repair."""
...
def main() -> int:
    ...
    notifier = TelegramNotifier.from_args(TelegramArgs(), run_label="kraken-dfrl-signal-state-repair")
    ...
    if notifier.enabled:
        notifier.send("DFRL signal-state repair started", f"PID={args.pid}\nRun root={root}")
    while process_running(args.pid):
        ...
        if key != last_key:
            ...
            if notifier.enabled and (stage != last_stage or bucket > last_bucket):
                notifier.send(
                    "DFRL signal-state repair progress",
        ...
        for threshold in (6, 8):
            if current_rss >= threshold * 1024**3 and threshold not in rss_alerts:
                ...
                if notifier.enabled:
                    notifier.send("DFRL repair RSS alert", f"RSS crossed {threshold} GiB\nPID={args.pid}\nRun root={root}")
        if time.time() - last_progress >= 3600 and not stalled_sent:
            stalled_sent = True
            if notifier.enabled:
                notifier.send("DFRL signal-state repair stalled", f"No heartbeat transition for 60 minutes\nPID={args.pid}\nRun root={root}")
    ...
    if decision_path.exists():
        ...
        title = "DFRL signal-state repair complete" if payload.get("status") == "complete" else "DFRL signal-state repair blocked"
        ...
        title = "DFRL signal-state repair failed or interrupted"
