...
if [[ "$LAUNCH" != "1" ]]; then
  ...
  echo "Suggested command: bash tools/run_qlmg_evidence_remediation_family_repair_tmux.sh --tmux-session-name ${SESSION} --stage all --resume --include-a2-a3 --include-b1 --include-c2 --include-branch-x --aggressive-overlay --require-telegram --seed 20260629 --launch-tmux"
  exit 0
fi
...
FULL_CMD=("$ROOT/.venv/bin/python" "tools/run_qlmg_evidence_remediation_family_repair.py" "--run-root" "$RUN_ROOT" "${ARGS[@]}")
SMOKE_CMD=("$ROOT/.venv/bin/python" "tools/run_qlmg_evidence_remediation_family_repair.py" "--stage" "all" "--smoke" "--max-symbols" "5" "--start" "2025-01-01" "--end" "2025-02-15" "--disable-telegram")
