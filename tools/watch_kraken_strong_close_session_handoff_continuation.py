#!/usr/bin/env python3
"""Detached Telegram/status watcher for the Strong-close session-handoff continuation v1."""
...
def main() -> int:
    ...
    notifier = TelegramNotifier.from_args(TelegramArgs(), run_label="kraken-strong-close-session-handoff-v1")
    ...
    if notifier.enabled:
        notifier.send("Strong-close session-handoff continuation v1 started", f"PID={args.pid}\nRun root={root}")
    while process_running(args.pid):
        ...
        if key != last_key:
            ...
            if notifier.enabled and (stage != last_stage or bucket > last_bucket):
                notifier.send(
                    "Strong-close session-handoff continuation v1 progress",
        ...
        for threshold in (6, 8):
            if current_rss >= threshold * 1024**3 and threshold not in rss_alerts:
                ...
                if notifier.enabled:
                    notifier.send("Strong-close session-handoff continuation v1 RSS alert", f"RSS crossed {threshold} GiB\nPID={args.pid}\nRun root={root}")
        if time.time() - last_progress >= 3600 and not stalled_sent:
            stalled_sent = True
            if notifier.enabled:
                notifier.send("Strong-close session-handoff continuation v1 stalled", f"No heartbeat transition for 60 minutes\nPID={args.pid}\nRun root={root}")
    ...
    if decision_path.exists():
        ...
        title = "Strong-close session-handoff continuation v1 complete" if payload.get("status") == "complete" else "Strong-close session-handoff continuation v1 blocked"
        ...
        title = "Strong-close session-handoff continuation v1 failed or interrupted"
