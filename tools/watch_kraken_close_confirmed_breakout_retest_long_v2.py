#!/usr/bin/env python3
"""Detached Telegram/status watcher for the Close-confirmed breakout-retest v2."""
...
def main() -> int:
    ...
    notifier = TelegramNotifier.from_args(TelegramArgs(), run_label="kraken-close-confirmed-breakout-retest-v2")
    ...
    if notifier.enabled:
        notifier.send("Close-confirmed breakout-retest v2 started", f"PID={args.pid}\nRun root={root}")
    while process_running(args.pid):
        ...
        if key != last_key:
            ...
            if notifier.enabled and (stage != last_stage or bucket > last_bucket):
                notifier.send(
                    "Close-confirmed breakout-retest v2 progress",
                    f"Stage={stage}\nSymbols={completed}\nRaw signals={progress.get('raw_signals', 0)}\n"
                    f"Accepted trades={progress.get('accepted_trade_rows', 0)}\nControl keys={progress.get('control_keys', 0)}\n"
        ...
        for threshold in (6, 8):
            if current_rss >= threshold * 1024**3 and threshold not in rss_alerts:
                ...
                if notifier.enabled:
                    notifier.send("Breakout-retest v2 RSS alert", f"RSS crossed {threshold} GiB\nPID={args.pid}\nRun root={root}")
        if time.time() - last_progress >= 3600 and not stalled_sent:
            stalled_sent = True
            if notifier.enabled:
                notifier.send("Close-confirmed breakout-retest v2 stalled", f"No heartbeat transition for 60 minutes\nPID={args.pid}\nRun root={root}")
    ...
    if decision_path.exists():
        ...
        title = "Close-confirmed breakout-retest v2 complete" if payload.get("status") == "complete" else "Close-confirmed breakout-retest v2 blocked"
        ...
        title = "Close-confirmed breakout-retest v2 failed or interrupted"
