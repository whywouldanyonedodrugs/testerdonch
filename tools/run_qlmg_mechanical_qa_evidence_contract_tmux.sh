...
if [[ "$LAUNCH" != "1" ]]; then
  ...
  echo "Suggested command: bash tools/run_qlmg_mechanical_qa_evidence_contract_tmux.sh --tmux-session-name ${SESSION} --stage all --resume --require-telegram --seed 20260630 --launch-tmux"
  exit 0
fi
...
FULL_CMD=("$ROOT/.venv/bin/python" "tools/run_qlmg_mechanical_qa_evidence_contract.py" "--run-root" "$RUN_ROOT" "${ARGS[@]}")
SMOKE_CMD=("$ROOT/.venv/bin/python" "tools/run_qlmg_mechanical_qa_evidence_contract.py" "--stage" "all" "--smoke" "--max-symbols" "5" "--start" "2025-01-01" "--end" "2025-02-15" "--disable-telegram")
