#!/usr/bin/env python3
"""Train-only close-confirmed rolling-range breakout retest long screen v2."""
...
RUN_ROOT = Path("results/rebaseline/phase_kraken_close_confirmed_breakout_retest_long_screen_20260715_v2")
AUDIT_ONLY_ROOT = Path("results/rebaseline/phase_kraken_close_confirmed_breakout_retest_long_screen_20260715_v1")
AUTHORITY_ROOT = Path("results/rebaseline/phase_kraken_rfbs_010_train_only_stability_review_20260715_v1")
START = pd.Timestamp("2023-01-01", tz="UTC")
PROTECTED = pd.Timestamp("2026-01-01", tz="UTC")
END = PROTECTED - pd.Timedelta(minutes=5)
CONTRACT_VERSION = "kraken_close_confirmed_breakout_retest_long_v2_20260715"
SIGNAL_STATE_CONTRACT_VERSION = signal_state.SIGNAL_STATE_CONTRACT_VERSION
RISK_MATCH_TOLERANCE_ATR = 0.25
CONTROL_CLASSES = (
    "immediate_same_breakout",
    "time_matched_delayed_without_retest",
    "rolling_level_reclaim_without_breakout",
    "generic_20bar_4h_breakout",
...
def policy_hash(row: Mapping[str, Any], *, include_exit: bool) -> str:
    fields = ["reference_bars", "retest_window_bars", "parent_policy"]
    if include_exit:
    ...
    vector.update({
        ...
        "reference": "highest_completed_4h_high_shift_1", "depth_limit_daily_atr": 0.75,
        "risk_band_daily_atr": [0.25, 1.5], "new_breakout_reset": "strictly_higher_reference_before_first_retest",
...
def frozen_manifest() -> pd.DataFrame:
    ...
    for reference_bars in (20, 60):
        for retest_window_bars in (3, 9):
            for parent_policy in ("both_up", "all_regime_comparator"):
                for exit_policy in ("fixed_72h", "daily_ema10_close", "swing_low_trail_7d"):
                    row = {
                        "definition_id": f"cbrl_v2_{len(rows)+1:03d}", "reference_bars": reference_bars,
                        "retest_window_bars": retest_window_bars, "parent_policy": parent_policy,
...
def raw_policy_hash(reference_bars: int, retest_window_bars: int) -> str:
    return stable_hash({
        "reference_bars": int(reference_bars), "retest_window_bars": int(retest_window_bars),
...
def feature_frames(bars: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for lookback in (20, 60):
        reference = frame.high.rolling(lookback, min_periods=lookback).max().shift(1)
        frame[f"range_high_{lookback}"] = reference
        frame[f"breakout_{lookback}"] = frame.close.gt(reference) & frame.close.shift(1).le(reference)
        frame[f"generic_breakout_{lookback}"] = frame.close.gt(reference)
        frame[f"level_reclaim_{lookback}"] = frame.low.le(reference) & frame.close.gt(reference) & ~frame[f"breakout_{lookback}"]
...
def anchored_vwap(work: pd.DataFrame, breakout_ts: pd.Timestamp, decision_ts: pd.Timestamp) -> float:
    selected = work[(work.known_ts > breakout_ts) & (work.known_ts <= decision_ts)]
...
def breakout_sequences(
    frame: pd.DataFrame, work: pd.DataFrame, reference_bars: int, retest_window_bars: int,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    """Resolve one setup without position state; only unresolved setup identity is deduplicated."""
    confirmed: list[dict[str, Any]] = []
    expired: list[dict[str, Any]] = []
    index = int(reference_bars)
    reference_col = f"range_high_{reference_bars}"
    breakout_col = f"breakout_{reference_bars}"
    while index < len(frame):
        breakout = frame.iloc[index]
        if not bool(breakout.get(breakout_col, False)) or pd.isna(breakout[reference_col]):
            index += 1
            continue
        breakout_index = index
        breakout_ts = pd.Timestamp(breakout.decision_ts)
        level = float(breakout[reference_col])
        retested = False
        retest_index: int | None = None
        sequence_low = np.inf
        cursor = index + 1
        resolved = False
        while cursor < len(frame) and cursor <= breakout_index + retest_window_bars:
            current = frame.iloc[cursor]
            current_atr = float(current.atr_14d) if pd.notna(current.atr_14d) else np.nan
            if not np.isfinite(current_atr) or current_atr <= 0:
                cursor += 1
                continue
            current_reference = current[reference_col]
            higher_reset = (
                not retested and pd.notna(current_reference) and float(current_reference) > level
                and float(current.close) > float(current_reference)
            )
            if higher_reset:
                breakout_index = cursor
                breakout_ts = pd.Timestamp(current.decision_ts)
                level = float(current_reference)
                sequence_low = np.inf
                cursor += 1
                continue
            sequence_low = min(sequence_low, float(current.low))
            if float(current.close) < level - 0.75 * current_atr:
                expired.append({"reason": "completed_close_beyond_depth_limit", "breakout_index": breakout_index, "resolution_index": cursor, "breakout_level": level})
                resolved = True
                index = cursor + 1
                break
            if float(current.low) <= level:
                retested = True
                retest_index = cursor if retest_index is None else retest_index
            elif retested and retest_index is not None and cursor > retest_index:
                vwap = anchored_vwap(work, breakout_ts, pd.Timestamp(current.decision_ts))
                if float(current.close) > level and pd.notna(vwap) and float(current.close) > vwap:
                    confirmed.append({
                        "breakout_index": breakout_index, "retest_index": retest_index, "decision_index": cursor,
                        "breakout_ts": breakout_ts, "breakout_level": level, "breakout_anchored_vwap": vwap,
            ...
            cursor += 1
        if not resolved:
            expired.append({"reason": "retest_reclaim_window_expired", "breakout_index": breakout_index, "resolution_index": min(cursor, len(frame)-1), "breakout_level": level})
...
def prepare_symbol(bars: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict[tuple[int, int], dict[str, list[dict[str, Any]]]]]:
    ...
    for reference_bars in (20, 60):
        for window in (3, 9):
            confirmed, expired = breakout_sequences(frame, work, reference_bars, window)
    ...
    for (reference_bars, window), sequence_group in sequences.items():
        ...
        for sequence in sequence_group["confirmed"]:
            ...
            row = {
                ...
                "retest_window_bars": window, "decision_ts": decision_ts, "feature_available_ts": decision.feature_available_ts,
                ...
                "breakout_ts": sequence["breakout_ts"], "breakout_level": sequence["breakout_level"],
                "breakout_anchored_vwap": sequence["breakout_anchored_vwap"],
            ...
            row["raw_signal_id"] = "CBRLRAW_" + row["raw_signal_address_hash"][:24]
            rows.append(row)
...
def project_parent_policies(raw: pd.DataFrame, definitions: pd.DataFrame) -> pd.DataFrame:
    ...
    for policy in definitions.drop_duplicates("selected_key_policy_hash").to_dict("records"):
        selected = raw[(raw.reference_bars == policy["reference_bars"]) & (raw.retest_window_bars == policy["retest_window_bars"])]
        ...
        for source in selected.to_dict("records"):
            ...
            row["candidate_key"] = "CBRLK_" + stable_hash({"policy": policy["selected_key_policy_hash"], "raw": source["raw_signal_address_hash"]})[:24]
            rows.append(row)
...
def simulate_definition(candidates: pd.DataFrame, definition: Mapping[str, Any], execute_fn: Callable) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for key in candidates.sort_values(["entry_ts", "symbol", "candidate_key"]).to_dict("records"):
        ...
        event["event_id"] = "CBRLE_" + stable_hash({"candidate": key["candidate_key"], "definition": definition["definition_id"]})[:24]
...
def raw_nesting_audit(raw: pd.DataFrame, projected: pd.DataFrame, definitions: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for (reference_bars, window), group in definitions.drop_duplicates("selected_key_policy_hash").groupby(["reference_bars", "retest_window_bars"]):
        ...
        raw_set = set(raw[(raw.reference_bars == reference_bars) & (raw.retest_window_bars == window)].raw_signal_address_hash)
        rows.append({"reference_bars": reference_bars, "retest_window_bars": window, "raw_rows": len(raw_set), "strict_rows": len(strict), "all_regime_rows": len(broad), "strict_not_in_all": len(strict-broad), "all_missing_known_raw": len(raw_set-broad), "pass": strict <= broad and broad == raw_set})
    return pd.DataFrame(rows)
...
def build_control_pool(frame: pd.DataFrame, data: dict[str, Any]) -> pd.DataFrame:
    ...
    pool["entry_ts"] = pd.NaT; pool["entry_price"] = np.nan
...
def build_control_keys(candidates: pd.DataFrame, outcomes: pd.DataFrame, feature_cache: dict[str, pd.DataFrame], bars_cache: dict[str, pd.DataFrame], indexed_cache: dict[str, dict[str, Any]], definitions: pd.DataFrame, progress_fn: Callable[[int, int], None] | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for number,key in enumerate(candidates.itertuples(index=False),1):
        ...
        breakout_row = frame[frame.decision_ts.eq(pd.Timestamp(key.breakout_ts))]
        if len(breakout_row):
            item = breakout_row.iloc[0]
            proposal = _exact_pool_choice(pool,pd.Timestamp(item.decision_ts))
            if proposal: proposals["immediate_same_breakout"].append(proposal)
            delayed_index = int(item.name) + int(key.retest_window_bars)
            if delayed_index < len(frame):
                ...
                if proposal: proposals["time_matched_delayed_without_retest"].append(proposal)
        choice=_pool_choice(pool,pool[f"level_reclaim_{int(key.reference_bars)}"].eq(True),pd.Timestamp(key.decision_ts),candidate_risk)
        if choice: proposals["rolling_level_reclaim_without_breakout"].append(choice)
        choice=_pool_choice(pool,pool.generic_breakout_20.eq(True),pd.Timestamp(key.decision_ts),candidate_risk)
        if choice: proposals["generic_20bar_4h_breakout"].append(choice)
        ...
        for control_class, options in proposals.items():
            ...
            for definition in policies[key.candidate_key].itertuples(index=False):
                ...
                control_key = "CBRLC_" + stable_hash(vector)[:24]
    ...
    if len(controls):
        controls = controls.sort_values(["definition_id", "control_class", "control_economic_address_hash", "candidate_key"]).drop_duplicates(["definition_id", "control_class", "control_economic_address_hash"])
    ...
    for definition_id in sorted(events.definition_id.unique()):
        ...
        for control_class in CONTROL_CLASSES:
            ...
            for mode in ("base", "conservative", "severe"):
                ...
                if control_class in {"immediate_same_breakout", "time_matched_delayed_without_retest"}:
...
def compact_bundle(root: Path) -> Path:
    files = (
        "SCREEN_REPORT.md", "decision_summary.json", "contract/close_confirmed_breakout_retest_contract.md",
        "manifest/definitions.csv", "signals/raw_signal_manifest.csv", "signals/parent_projected_manifest.csv",
        ...
        "audit/hard_gate_audit.csv", "economics/definition_summary.csv", "economics/period_summary.csv",
        "forensics/concentration_and_removal.csv", "forensics/parameter_neighborhood.csv", "forensics/exact_vs_imputed_funding.csv",
        "forensics/horizon_path_behavior.csv", "forensics/leave_one_symbol.csv", "forensics/leave_one_month.csv",
...
def run(root: Path) -> dict[str, Any]:
    ...
    authority = json.loads((AUTHORITY_ROOT / "continuity/continuity_state_snapshot.json").read_text())
    if authority.get("next_authorized_target") != "Close-confirmed breakout retest long screen" or not authority.get("paused_family_authorized"):
        raise RuntimeError("family launch is not authorized by immutable continuity state")
    source_hash_before = directory_hash(AUDIT_ONLY_ROOT)
    ...
    contract_text = """# Close-Confirmed Breakout Retest Long v2 Contract

Train-only 2023-2025. The rolling 20/60 completed-4h high uses `rolling.max().shift(1)`. A completed close must cross above the frozen level. One unresolved sequence is held per symbol/reference profile; before its first retest only, a completed close above a strictly higher pre-decision rolling reference resets it. A later bar must touch the level without closing more than 0.75 completed-daily ATR below it, and a still later completed bar must close above the frozen level and breakout-anchored VWAP within three/nine completed bars. Entry is the next executable 5m open. Stop is the complete retest-sequence low and risk must be 0.25-1.5 completed-daily ATR. Parent-neutral raw tapes are frozen before PIT both-up/all-known projections. Non-overlap is definition-local and uses actual executable exits. No funding, OI, rank, impulse, compression or prior-high distance activates a signal. Evaluation-boundary crossings are excluded, never artificially exited. Controls use the same risk band and a frozen +/-0.25 daily-ATR risk-distance rule before key freeze. This is not A1, prior-high-v2 or legacy A3.
"""
    contract_path = root / "contract/close_confirmed_breakout_retest_contract.md"; contract_path.parent.mkdir(); contract_path.write_text(contract_text, encoding="utf-8")
    ...
    neighborhood = outcomes.groupby(["reference_bars","retest_window_bars","parent_policy","exit_policy"]).agg(events=("event_id","size"),symbols=("symbol","nunique"),months=("entry_ts",lambda x:pd.to_datetime(x,utc=True).dt.strftime("%Y-%m").nunique()),base_mean_R=("net_base_R","mean"),conservative_mean_R=("net_conservative_R","mean"),severe_mean_R=("net_severe_R","mean")).reset_index(); write_csv(root / "forensics/parameter_neighborhood.csv", neighborhood)
    write_csv(root / "forensics/exact_vs_imputed_funding.csv", reports.funding_partition_report(outcomes)); write_csv(root / "forensics/horizon_path_behavior.csv", indexed_path_report(outcomes,indexed_cache))
    ...
    artificial=int(outcomes.artificial_horizon_exit.sum()); boundary_reconciliation=[{"scope":"raw_boundary_drops","count":len(raw_drops)},{"scope":"definition_outcome_exclusions","count":len(exclusions)},{"scope":"accepted_interval_violations","count":len(interval_violations)},{"scope":"artificial_endpoint_exits","count":artificial}]; write_csv(root / "audit/boundary_reconciliation.csv",boundary_reconciliation)
    eligible_rows=contract_manifest["eligible_definition_rows"]
    hard={"definitions_evaluated":int(summary.definition_id.nunique()),"raw_policy_hashes":raw.raw_policy_hash.nunique(),"selected_key_policy_hashes":projected.selected_key_policy_hash.nunique(),"exactness_sentinel_rows":len(sentinel),"exactness_sentinel_failures":int((~sentinel["pass"]).sum()),"raw_duplicates":int(raw.duplicated(["raw_policy_hash","raw_signal_address_hash"]).sum()),"candidate_duplicate_addresses":int(outcomes.duplicated(["definition_id","candidate_economic_address_hash"]).sum()),"control_duplicate_addresses":int(control_keys.duplicated(["definition_id","control_class","control_economic_address_hash"]).sum()) if len(control_keys) else 0,"strict_parent_nesting_failures":int((~nesting["pass"]).sum()),"deterministic_replay_mismatches":parity["mismatch_count"],"unexplained_attrition":int(eligible_rows!=len(outcomes)+len(skips)+len(exclusions)),"artificial_endpoint_exits":artificial,"evaluation_interval_violations":len(interval_violations),"candidate_funding_join_missing":int(boundaries.missing.sum()) if len(boundaries) else 0,"candidate_funding_join_duplicates":int(boundaries.duplicated(["event_id","boundary_ts"]).sum()) if len(boundaries) else 0,"control_funding_join_missing":int(control_boundaries.missing.sum()) if len(control_boundaries) else 0,"control_funding_join_duplicates":int(control_boundaries.duplicated(["control_event_id","boundary_ts"]).sum()) if len(control_boundaries) else 0,"decision_input_leaks":int((raw.feature_available_ts>raw.decision_ts).sum())+(int((control_keys.feature_available_ts>control_keys.decision_ts).sum()) if len(control_keys) else 0),"protected_period_violations":int(outcomes.protected_violation.sum()),"placeholder_controls":int(control_keys.placeholder_control.sum()) if len(control_keys) else 0,"matched_unmatched_failures":int(((matched_bias.matched_count+matched_bias.unmatched_count)!=matched_bias.full_count).sum()) if len(matched_bias) else 0,"control_outcomes_before_freeze":int(control_keys.outcome_accessed_before_freeze.sum()) if len(control_keys) else 0,"imputed_funding_gate_activations":int(raw.imputed_funding_gate_activated.sum()),"audit_only_root_mutations":int(directory_hash(AUDIT_ONLY_ROOT)!=source_hash_before)}
