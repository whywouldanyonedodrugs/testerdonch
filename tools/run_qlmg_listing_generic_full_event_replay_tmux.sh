...
BASE_RUN_ROOT="$ROOT_DIR/results/rebaseline/phase_qlmg_listing_generic_full_event_replay_20260627_v1"
if [[ -n "${RUN_ROOT:-}" ]]; then
  RUN_ROOT="$RUN_ROOT"
...
SESSION="qlmg_listing_full_replay"
SKIP_SMOKE=0
LAUNCH_TMUX=0
ARGS=()
while [[ $# -gt 0 ]]; do
...
EOF
SMOKE_CMD="cd '$ROOT_DIR' && ./.venv/bin/python tools/run_qlmg_listing_generic_full_event_replay.py --stage all --smoke --max-symbols 5 --start 2025-01-01 --end 2025-02-15 --disable-telegram --run-root '${RUN_ROOT}_smoke'"
FULL_CMD="cd '$ROOT_DIR' && ./.venv/bin/python tools/run_qlmg_listing_generic_full_event_replay.py --run-root '$RUN_ROOT' ${ARGS[*]}"
...
if [[ "$LAUNCH_TMUX" -ne 1 ]]; then
  ...
  echo "bash tools/run_qlmg_listing_generic_full_event_replay_tmux.sh --tmux-session-name $SESSION --stage all --resume --download-targeted-1m --targeted-download-cap-gb 30 --use-existing-1m-if-overlap --include-d4-reconciliation --include-generic-shock --include-secondary-preservation --require-telegram --seed 20260627 --launch-tmux" | tee -a "$LOG"
  cat > "$RUN_ROOT/watch_status.json" <<EOF
