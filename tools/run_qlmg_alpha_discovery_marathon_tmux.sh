...
BASE_RUN_ROOT="$ROOT_DIR/results/rebaseline/phase_qlmg_alpha_discovery_marathon_20260625_v1"
if [[ -n "${RUN_ROOT:-}" ]]; then
  RUN_ROOT="$RUN_ROOT"
...
EOF
SMOKE_CMD="cd '$ROOT_DIR' && ./.venv/bin/python tools/run_qlmg_alpha_discovery_marathon.py --stage all --smoke --max-symbols 5 --start 2025-01-01 --end 2025-02-15 --disable-telegram --discovery-budget 40 --refine-budget 8 --nulls-per-event 1 --run-root '${RUN_ROOT}_smoke'"
FULL_CMD="cd '$ROOT_DIR' && ./.venv/bin/python tools/run_qlmg_alpha_discovery_marathon.py --run-root '$RUN_ROOT' ${ARGS[*]}"
...
if [[ "$LAUNCH_TMUX" -ne 1 ]]; then
  ...
  echo "bash tools/run_qlmg_alpha_discovery_marathon_tmux.sh --tmux-session-name $SESSION --stage all --resume --discovery-budget 1800 --refine-budget 240 --nulls-per-event 3 --use-targeted-1m-if-overlap --require-telegram --seed 20260625 --launch-tmux" | tee -a "$LOG"
  cat > "$RUN_ROOT/watch_status.json" <<EOF
