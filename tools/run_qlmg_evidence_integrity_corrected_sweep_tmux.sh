...
if [[ "$LAUNCH" != "1" ]]; then
  ...
  echo "Suggested command: bash tools/run_qlmg_evidence_integrity_corrected_sweep_tmux.sh --tmux-session-name ${SESSION} --stage all --resume --include-a2-a3 --include-b1 --include-c2 --include-branch-x --nulls-per-event 3 --top-per-family 40 --corrected-sweep-budget 1200 --aggressive-overlay --require-telegram --seed 20260628 --launch-tmux"
  exit 0
fi
...
FULL_CMD=("$ROOT/.venv/bin/python" "tools/run_qlmg_evidence_integrity_corrected_sweep.py" "--run-root" "$RUN_ROOT" "${ARGS[@]}")
SMOKE_CMD=("$ROOT/.venv/bin/python" "tools/run_qlmg_evidence_integrity_corrected_sweep.py" "--stage" "all" "--smoke" "--max-symbols" "5" "--start" "2025-01-01" "--end" "2025-02-15" "--disable-telegram" "--nulls-per-event" "1" "--top-per-family" "5" "--corrected-sweep-budget" "80")
