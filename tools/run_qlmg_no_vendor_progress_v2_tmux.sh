...
cd "$ROOT"
SESSION="qlmg_no_vendor_progress_v2"
LAUNCH=0
SKIP_SMOKE=0
EXPLICIT_RUN_ROOT=""
REQUIRE_TELEGRAM=0
...
if [[ "$LAUNCH" != "1" ]]; then
  ...
  echo "Suggested command: bash tools/run_qlmg_no_vendor_progress_v2_tmux.sh --tmux-session-name ${SESSION} --stage all --resume --include-funding-mark-enrichment --include-a1-a4-sweep --include-c2-expansion --include-branch-x-capture-calibration --a1-budget 2000 --a4-budget 2000 --a3-budget 1000 --nulls-per-event 3 --top-per-family 80 --require-telegram --seed 20260630 --launch-tmux"
  exit 0
fi
...
FULL_CMD=("$ROOT/.venv/bin/python" "tools/run_qlmg_no_vendor_progress_v2.py" "--run-root" "$RUN_ROOT" "${ARGS[@]}")
SMOKE_CMD=("$ROOT/.venv/bin/python" "tools/run_qlmg_no_vendor_progress_v2.py" "--stage" "all" "--smoke" "--max-symbols" "5" "--start" "2025-01-01" "--end" "2025-02-15" "--disable-telegram" "--a1-budget" "20" "--a4-budget" "20" "--a3-budget" "10" "--nulls-per-event" "1" "--top-per-family" "5")
